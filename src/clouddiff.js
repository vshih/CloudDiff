...
import { EditorView, codeMirrorSetup } from './codemirror-setup'
...
function exDiffResponseHandler(response) {
	switch (response) {
		case 'OK':
		case undefined:
...
class Diff {
	...
	async diffOnClick(source_element) {
		...
		try {
			...
			if (!(err instanceof IgnoreException)) {
				alert(`CloudDiff failed with the following error
					(the Console of CloudDiff's service worker may have more information):`, err);
			}
			return;
		}
		...
		if (source_element.id === 'clouddiff-indiff') {
			...
			chrome.runtime.sendMessage(
				ex_data,
				response => {
					exDiffResponseHandler(response);
				}
			);
		}
	}
